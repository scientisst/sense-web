...
export class ESPLoader {
		...
		for (let i = 0; i < 100; i++) {
			...
			if (resp == 1) {
				if (op == null || op_ret == op) {
					...
					data[0] != 0 &&
					data[1] == this.ROM_INVALID_RECV_MSG
				) {
	...
	async flash_begin(size: number, offset: number) {
		...
		pkt = this._appendArray(
			pkt,
			this._int_to_bytearray(this.FLASH_WRITE_SIZE)
		)
		pkt = this._appendArray(pkt, this._int_to_bytearray(offset))
		if (this.IS_STUB == false) {
	...
	async main_fn(mode = "default_reset") {
		...
		this.info(
			"Crystal is " + (await this.chip.get_crystal_freq(this)) + "MHz"
		)
	...
	flash_size_bytes = function (flash_size: string) {
		...
		if (flash_size.indexOf("KB") !== -1) {
			...
			flash_size_b =
				parseInt(flash_size.slice(0, flash_size.indexOf("MB"))) *
				1024 *
				1024
		}
		return flash_size_b
	...
	async write_flash(
		...
		if (flash_size !== "keep") {
			...
			for (let i = 0; i < fileArray.length; i++) {
				if (
					fileArray[i].data.length + fileArray[i].address >
					flash_end
				) {
					throw new ESPError(
		...
		for (let i = 0; i < fileArray.length; i++) {
			...
			if (compress) {
				...
				blocks = await this.flash_defl_begin(
					uncsize,
					image.length,
					address
				)
			...
			while (image.length > 0) {
				...
				const block = this.bstrToUi8(
					image.slice(0, this.FLASH_WRITE_SIZE)
				)

				if (compress) {
					const len_uncompressed_previous = total_len_uncompressed
					inflate.push(block, false)
					const block_uncompressed =
						total_len_uncompressed - len_uncompressed_previous
					let block_timeout = 3000
					if (
						this.timeout_per_mb(
							this.ERASE_WRITE_TIMEOUT_PER_MB,
							block_uncompressed
						) > 3000
					) {
						block_timeout = this.timeout_per_mb(
							this.ERASE_WRITE_TIMEOUT_PER_MB,
							block_uncompressed
						)
					}
			...
			if (calcmd5) {
				...
				if (
					new String(res).valueOf() != new String(calcmd5).valueOf()
				) {
	...
	async flash_id() {
		...
		this.info(
			"Device: " +
				...
				flid_lowbyte.toString(16)
		)
		this.info(
			"Detected flash size: " + this.DETECTED_FLASH_SIZES[flid_lowbyte]
		)
	}
