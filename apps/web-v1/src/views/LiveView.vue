	...
	<div class="live">
		...
		<div class="buttons">
			...
			<div v-if="connected">
				...
				<button v-else class="button" id="start" @click="start">
					start
				</button>
			</div>
		...
		onSerialData(data) {
			...
			if (this.live) {
				...
				if (this.firstColIsTime) {
					if (this.micros) {
						timestamp =
							Math.floor(parsedData[0] / 1000) - this.startTime
					} else {
		...
		async connect() {
			...
			if (this.device == 0) {
				...
				if (this.scientisst != null) {
					...
					this.senseVersion = this.scientisst.version.replace(
						"ScientISST",
						""
					)
				}
		...
		getMetadata() {
			...
			if (this.device == 0) {
				...
				return {
					...
					"Channels labels": this.channels.map(
						channel => "AI" + channel
					),
					Header: this.getHeader().split("\t"),
					"ISO 8601": iso,
