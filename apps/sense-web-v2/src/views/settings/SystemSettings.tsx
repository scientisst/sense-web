import { ButtonRadioGroupField, ImageButton, TextButton, TextField } from "@scientisst/react-ui/components/inputs";
import Table from "../../components/Table";
import { eventProps } from "../../utils/constants";
import { useEffect } from "react";
import { Field, FieldArray } from "formik";
import { Box, Button, CheckboxGroup, FormControl, FormLabel, IconButton, Input } from "@chakra-ui/react";
            ...
            {events.map((event, index) => (
                <Box key={index} display="flex" alignItems="center" marginBottom="1rem">
                        ...
                        {({ field, form }) => (
                            ...
                            <>
                                <input 
                                    ...
                                    onChange={(event) => {form.setFieldValue(field.name, convertHexToRgb(event.target.value))}}
                                    style={{
                                        marginTop: '1.5rem',
                                        width: '54px',
                                        height: '54px',
                    ...
                    <IconButton
                        ...
                        icon={<FontAwesomeIcon icon={faTrash} className="h-6 w-6" />}
                        aria-label="Delete"
            ...
            <FieldArray name="eventsLabel">
            {
                ({ form, push, remove }) => {
                    ...
                    return (
                        <>
                            <Box mb={5}>
                                <EventsTable events={eventsLabel} remove={remove}/>
                            </Box>

                            <TextButton size={"base"} onClick={() => push(newEvent(eventsLabel))}>Create event</TextButton>
                        </>
                    );
                }
            }
