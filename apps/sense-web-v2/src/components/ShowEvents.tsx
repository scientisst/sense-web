...
const EventsLabel = ({ eventsLabel, style }) => {
	const toggleEvents = eventsLabel.filter(event => event.toggle === "true")
	const holdEvents = eventsLabel.filter(event => event.toggle === "false")
	...
	const transformIntoJsx = (events, label) => {
		...
		const eventsJsx = events.map((event, index) => (
			...
			>
				...
				/>
				<Box alignItems="center" textUnderlineOffset={1}>
					{event.name}
				</Box>
